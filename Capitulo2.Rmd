...
library(tidyverse) 
seq(-2,2,by=.01) %>%   expand.grid(x=., y=.) %>%   ggplot(aes(x=(x^3-sin(y)), y=(y^3-cos(x)))) +   geom_point(alpha=.05, shape=20, size=0)+   theme_void()+   coord_polar()
```

## Mas figuras

```{r figra2, echo=FALSE, message=FALSE, warning=FALSE}
